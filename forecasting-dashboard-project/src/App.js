...
export default function App() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            setError('Auth Client Init Error: Check Client ID.');
            setAuthClientReady(false);
        }
    ...
    const handleAuthClick = useCallback(() => {
        ...
        if (authClientReady && tokenClient.current) {
            ...
            setError('Auth client not ready. Please wait or check credentials.');
        }
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (result.values && result.values.length > 1) { // Check for header + data
                    ...
                    if (JSON.stringify(header) !== JSON.stringify(expectedHeader)) {
                        setError("Header mismatch. Ensure columns are: " + expectedHeader.join(', '));
                        return;
                    }
                    ...
                    const parsedData = result.values.slice(1).map((row, index) => {
                        ...
                        try {
                            ...
                            throw new Error(`Parsing error on row ${rowNum}. Check data format.`);
                        }
                    });
                    setData(parsedData);
                    setError(null);
                } else { setError("No data found in Sheet1. Ensure data starts on row 2."); }
            } catch (err) {
                const apiError = err.result?.error?.message;
                setError(apiError || "Fetch Error. Check sheet permissions and URL.");
                console.error(err);
            }
        };
        fetchData();
